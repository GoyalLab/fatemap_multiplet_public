...
save_root<-"/projects/b1042/GoyalLab/zzhang/doublet_objects_sum/hybrid"
...
# only rerun Biorxiv, LARRY, ClonMapper , smartseq3-reads
all_datasets_dirs<-list.dirs(path = root_dir, full.names = TRUE, recursive = F)
out_df<-NULL
...
for(cur_dataset_dir in all_datasets_dirs){
  ...
  out1 <- run_hybrid_on_dataset(cur_dataset_dir, "hybrid", dbl_act = ACT_DBL, dbl_exp = 0.05, num_to_test = 1, save_root = save_root)
  out1$dbl_exp <- 0.05
  out1$dbl_act <- ACT_DBL
  out2 <- run_hybrid_on_dataset(cur_dataset_dir, "hybrid", dbl_act = ACT_DBL, dbl_exp = 0.08, num_to_test = 1, save_root = save_root)
  out2$dbl_exp <- 0.08
  out2$dbl_act <- ACT_DBL
  out3 <- run_hybrid_on_dataset(cur_dataset_dir, "hybrid", dbl_act = ACT_DBL, dbl_exp = 0.1, num_to_test = 1, save_root = save_root)
  out3$dbl_exp <- 0.1
  out3$dbl_act <- ACT_DBL
  out4 <- run_hybrid_on_dataset(cur_dataset_dir, "hybrid", dbl_act = ACT_DBL, dbl_exp = 0.15, num_to_test = 1, save_root = save_root)
  out4$dbl_exp <- 0.15
  out4$dbl_act <- ACT_DBL
  out5 <- run_hybrid_on_dataset(cur_dataset_dir, "hybrid", dbl_act = ACT_DBL, dbl_exp = 0.2, num_to_test = 1, save_root = save_root)
  out5$dbl_exp <- 0.2
  out5$dbl_act <- ACT_DBL
  out6 <- run_hybrid_on_dataset(cur_dataset_dir, "hybrid", dbl_act = ACT_DBL, dbl_exp = 0.25, num_to_test = 1, save_root = save_root)
  out6$dbl_exp <- 0.25
