...
run_bchmk_original_paper <- function(cur_bchmk_dataset_mtx, num_to_test){
  ...
  for(idx in 1:num_test){
    ...
    cur_file <- glue("/projects/b1042/GoyalLab/zzhang/doublet_objects_benchmarking_data/hybrid/{cur_data_id}.rds")
    cur_res <- run_cxds(sce, exp_dbl = cur_exp_dbl, doublet_obj_file = cur_file)
...
run_cxds <- function(sce, exp_dbl, doublet_obj_file) {
  ...
  sce <- cxds_bcds_hybrid(sce, estNdbl = T, force=T)
