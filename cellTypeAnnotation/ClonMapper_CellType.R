...
library(svglite)
#Path to directories
inputDirectory <- "~/Keerthana/CellTypeCountData/AdditionalData/ClonMapper_FinalData/"
outputDirectoryData <- "~/Keerthana/CellTypeCountData/plotData/ClonMapper/"
outputDirectoryPlots <- "~/Keerthana/CellTypeCountData/plots/ClonMapper/"
outputRDS  <- "~/Keerthana/CellTypeCountData/finalRDS/ClonMapper/"
...
counts <- Read10X(data.dir = paste0(inputDirectory, "10X/",sampleName,"/"))
...
sample <- subset(sample, cells = colnames(sample)[which(sample$nCount_RNA > countLower)])
...
sample <- RunPCA(object = sample, verbose = T, npcs = 40, seed.use = seed.use)

sample <- FindNeighbors(object = sample, verbose = T, dims = 1:40)

sample <- RunUMAP(object = sample, verbose = T, seed.use = seed.use, dims = 1:40)
...
db = paste0(inputDirectory, "/ClonMapper-scTypeMarkers.xlsx")
tissue = "Blood"
