...
inputDirectoryPaper = "~/Keerthana/CellCounts/UnprocessedData/FateMap/FM05/"
...
inputDirectorySingletCode = "~/Keerthana/CellCounts/SingletCodeData/FM05/"
...
samples <- c("FM05-250nMPLX-A1", "FM05-250nMPLX-A2")
...
for (index in 1:length(samples)){
  ...
  #Data 8
  numPaperSingletsQC <- append(numPaperSingletsQC, filterQC(samplePaper, featuresLower, featuresUpper, percentMito, save = TRUE, outputPath = paste0(outputRDSPaper, "FateMap_FM05_", sampleName, ".rds")))
  ...
  #Data 10
  numTrueSingletsQC <- append(numTrueSingletsQC, filterQC(sampleSingletCode, featuresLower, featuresUpper, percentMito, save = TRUE, outputPath = paste0(outputRDSSingletCode, "FateMap_FM05_", sampleName, ".rds")))
}
...
write.csv(CellNumbers, paste0(outputNumbers, "FateMap_FM05.csv"))
...
doubletFiltering <- function(barcodes10X){
  ...
  umiCut = max(3, round(3*totUMI/(4e5)))
