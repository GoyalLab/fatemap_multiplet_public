...
inputDirectoryPaper = "~/Keerthana/CellCounts/UnprocessedData/FateMap/FMXX/"
...
inputDirectorySingletCode = "~/Keerthana/CellCounts/SingletCodeData/FM08/"
...
#Not running filtered_feature_bc_matrix3 because singlet code is not run on it
fileNamePaper = c("filtered_feature_bc_matrix", "filtered_feature_bc_matrix2")


samples <- c("run1_sample3", "run1_sample4")
...
for (index in 1:length(samples)){
  ...
  #Data 8
  numPaperSingletsQC <- append(numPaperSingletsQC, filterQC(samplePaper, featuresLower, featuresUpper, percentMito, save = TRUE, outputPath = paste0(outputRDSPaper, "FateMap_FM08_", sampleName, ".rds")))
  ...
  #Data 10
  numTrueSingletsQC <- append(numTrueSingletsQC, filterQC(sampleSingletCode, featuresLower, featuresUpper, percentMito, save = TRUE, outputPath = paste0(outputRDSSingletCode, "FateMap_FM08_", sampleName, ".rds")))
}
...
write.csv(CellNumbers, paste0(outputNumbers, "FateMap_FM08.csv"))
