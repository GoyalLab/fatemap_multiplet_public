...
inputDirectoryPaper = "~/Keerthana/CellCounts/UnprocessedData/FateMap/FM03/"
...
inputDirectorySingletCode = "~/Keerthana/CellCounts/SingletCodeData/FM03/"
...
samples <- c("DMSO-FM3-1uMPLX", "DOT1Li-FM3-1uMPLX")
...
for (index in 1:length(samples)){
  ...
  #Data 8
  numPaperSingletsQC <- append(numPaperSingletsQC, filterQC(samplePaper, featuresLower, featuresUpper, percentMito, save = TRUE, outputPath = paste0(outputRDSPaper, "FateMap_FM03_", sampleName, ".rds")))
  ...
  #Data 8
  numTrueSingletsQC <- append(numTrueSingletsQC, filterQC(sampleSingletCode, featuresLower, featuresUpper, percentMito, save = TRUE, outputPath = paste0(outputRDSSingletCode, "FateMap_FM03_", sampleName, ".rds")))
}
...
write.csv(CellNumbers, paste0(outputNumbers, "FateMap_FM03.csv"))
...
umiFilter <- function(df){
  ...
  df <- df %>%
    ...
    ungroup()
  totUMI <- sum(df$nUMI)
  umiCutoff = max(15, round(3*totUMI/(4e5)))
  dfGood <- df %>%
