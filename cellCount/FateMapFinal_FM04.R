...
inputDirectoryPaper = "~/Keerthana/CellCounts/UnprocessedData/FateMap/FM04/"
...
inputDirectorySingletCode = "~/Keerthana/CellCounts/SingletCodeData/FM04/"
...
for (index in 1:length(samples)){
  ...
  #Data 8
  numPaperSingletsQC <- append(numPaperSingletsQC, filterQC(samplePaper, featuresLower, featuresUpper, percentMito, save = TRUE, outputPath = paste0(outputRDSPaper, "FateMap_FM04_", sampleName, ".rds")))
  ...
  #Data 8
  numTrueSingletsQC <- append(numTrueSingletsQC, filterQC(sampleSingletCode, featuresLower, featuresUpper, percentMito, save = TRUE, outputPath = paste0(outputRDSSingletCode, "FateMap_FM04_", sampleName, ".rds")))
}
...
write.csv(CellNumbers, paste0(outputNumbers, "FateMap_FM04.csv"))
...
doubletFiltering <- function(barcodes10X){
  ...
  umiCut = max(6, round(3*totUMI/(4e5)))
