...
barcodesAll <- read.csv(paste0(inputDirectoryPaper, "all_brains_cellID_barcode_UMI.csv"))

#Input and output for singletCode
inputDirectorySingletCode = "~/Keerthana/CellCounts/SingletCodeData/smartseq3_umis/"
...
for (index in 1:length(samples)){
  ...
  # QcPlot
  # fScatPlot<-FeatureScatter(samplePaper, feature1 = "nCount_RNA", feature2 = "nFeature_RNA") + geom_vline(xintercept=130000) + geom_hline(yintercept=11000) + geom_hline(yintercept=2500)
  # fScatPlot
  # ggsave(plot = QcPlot, filename = paste0(outputPlotPaper, "smartseq3UMIQC_", sampleName,".png"), width = 6, height = 6)
  # ggsave(plot = fScatPlot, filename = paste0(outputPlotPaper, "smartseq3UMIQC2_", sampleName,".png"), width = 6, height = 6)
  #Data 2
  ...
  #Data 8
  numPaperSingletsQC <- append(numPaperSingletsQC, filterQC(samplePaper, featuresLower, featuresUpper, countsUpper, mitoPercent, save = TRUE, outputPath = paste0(outputRDSPaper, "smartseq3UMI_", sampleName, ".rds")))
  ...
  #Data 10
  numTrueSingletsQC <- append(numTrueSingletsQC, filterQC(sampleSingletCode, featuresLower, featuresUpper, countsUpper, mitoPercent, save = TRUE, outputPath = paste0(outputRDSSingletCode, "smartseq3UMI_", sampleName, ".rds")))
}
...
write.csv(CellNumbers, paste0(outputNumbers, "smartseq3UMI.csv"))
