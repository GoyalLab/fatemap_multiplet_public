...
inputDirectoryPaper = "~/Keerthana/CellCounts/UnprocessedData/FateMap/FM01/"
...
inputDirectorySingletCode = "~/Keerthana/CellCounts/SingletCodeData/FM01/"
...
for (index in 1:length(samples)){
  ...
  #Data 8
  numPaperSingletsQC <- append(numPaperSingletsQC, filterQC(samplePaper, featuresLower, featuresUpper, percentMito, save = TRUE, outputPath = paste0(outputRDSPaper, "FateMap_FM01_", sampleName, ".rds")))
  # Getting 6 from supp table 11 from the paper
  ...
  #Data 8
  numTrueSingletsQC <- append(numTrueSingletsQC, filterQC(sampleSingletCode, featuresLower, featuresUpper, percentMito, save = TRUE, outputPath = paste0(outputRDSSingletCode, "FateMap_FM01_", sampleName, ".rds")))
}
...
write.csv(CellNumbers, paste0(outputNumbers, "FateMap_FM01.csv"))
...
umiFilter <- function(df){
  ...
  df <- df %>%
    ...
    ungroup()
  totUMI <- sum(df$nUMI)
  umiCutoff = max(4, round(3*totUMI/(4e5)))
  dfGood <- df %>%
