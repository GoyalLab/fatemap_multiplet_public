...
def count_doublets(input_file, output_prefix, umi_cutoff_ratio=3 / 4e5, umi_diff_threshold=50, dominant_threshold=10,
                   overwrite=False, min_umi_good_data_cutoff = 2):
    cur_singlet_stats = []
    df = pd.read_csv(input_file, sep='\t')
    singlets_file = output_prefix + "_singlets.txt"
    multiplets_file = output_prefix + "_multiplets.txt"
    singlet_stats_file = output_prefix + "_singlets_stats.csv"
    simulated_doublets_file = output_prefix + "_simulated_doublets.txt"
    ...
    for cur_sample_num in df["sampleNum"].unique():
        ...
        cur_umi_adjusted_cutoff = max(round(umi_cutoff_ratio * cur_sample_df.shape[0]), min_umi_good_data_cutoff)
    ...
    good_data["label"] = good_data["cellID"].apply(lambda x: "Singlet" if x in singlets_step3 else "Multiplet")

    # generate singlet pair for each sample for doublet simulation
    for cur_sample_num in good_data["sampleNum"].unique():
        cur_good_data = good_data[good_data["sampleNum"] == cur_sample_num]
        cur_singlets = good_data[good_data["label"] == "Singlet"]["cellID"].values
