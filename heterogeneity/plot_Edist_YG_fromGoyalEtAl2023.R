...
dataDirectory <- "/Volumes/fsmresfiles/Basic_Sciences/CDB/GoyalLab/People/MadelineMelzer/ZhangMelzerEtAl/data/heterogeneity/EDistance/results/"
...
dfNumberClusters <- tibble(read.table(file = paste0(dataDirectory, "NumberClusters_fm01s1_SPLINTRretransplant.csv"), header = TRUE, stringsAsFactors=F, sep = ",", fill = TRUE))
dfDistance<- tibble(read.table(file = paste0(dataDirectory,"Edistances_fm01s1_SPLINTRretransplant.csv"), header = TRUE, stringsAsFactors=F, sep = ",", fill = TRUE))
...
wilcox.test(df_joinClusterSpread$FM01, df_joinClusterSpread$SPLINTR, paired = TRUE, alternative = "two.sided", )
...
dfMeanDistance <- dfDistance %>% group_by(Dataset,resolution) %>% dplyr::summarise(mean = mean(distances)) 
...
ggsave(plot, file = paste0(plotDirectory, "resolution_NumberCluster_FM01s1_SPLINTRretransplant.svg"), width = 6, height = 4)
ggsave(plot, file = paste0(plotDirectory, "resolution_NumberCluster_FM01s1_SPLINTRretransplant.png"), width = 6, height = 4)
...
for (i in c(1,2,3,4,5,6,7,8,9))
{
  ...
  dfDistanceFilter6 <- dfDistanceSample %>% filter((Dataset== "SPLINTR") & (numberCluster == i))
...
plot = ggplot(dfDistancePlot,aes(x=Dataset, y=distances, color = Dataset)) +
  ...
  theme(axis.line = element_line(colour = "black"), panel.border=element_blank(), 
        panel.background=element_blank(), panel.grid.major=element_blank(), panel.grid.minor=element_blank())
plot
ggsave(plot, file = paste0(plotDirectory, "numberCluster_Edist_connectingLines_fm01s1_SPLINTRretransplant.svg"), width = 6, height = 4)
